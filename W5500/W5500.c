...
////	
////    GPIO_InitStructure1.GPIO_Pin = GPIO_Pin_13;				 //LED2-->PC13 端口配置
////    GPIO_InitStructure1.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
////    GPIO_InitStructure1.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
////    GPIO_Init(GPIOC, &GPIO_InitStructure1);					 //根据设定参数初始化PC13
////    GPIO_ResetBits(GPIOC,GPIO_Pin_13);						    //PC13输高
...
//	SPI_Cmd(SPI1,ENABLE);	//STM32使能SPI1
	...
	/* 初始化配置STM32 SPI1 */
	hspi1.Instance=SPI1;
	hspi1.Init.Direction=SPI_DIRECTION_2LINES;	//SPI设置为双线双向全双工
	hspi1.Init.Mode=SPI_MODE_MASTER;							//设置为主SPI
	hspi1.Init.DataSize=SPI_DATASIZE_8BIT;						//SPI发送接收8位帧结构
	hspi1.Init.CLKPolarity=SPI_POLARITY_LOW;							//时钟悬空低
	hspi1.Init.CLKPhase = SPI_PHASE_1EDGE;							//数据捕获于第1个时钟沿
	hspi1.Init.NSS = SPI_NSS_SOFT;							//NSS由外部管脚管理
	...
	{
		Error_Handler();
	}								//根据SPI_InitStruct中指定的参数初始化外设SPI1寄存器
